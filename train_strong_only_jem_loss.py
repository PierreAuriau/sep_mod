...
def train(weak_encoder, strong_encoder, train_loader, test_loader, n_epochs, pond, ind):
    # define optimizer
        ...
        for batch_idx, (weak_view, strong_view, _, _) in enumerate(train_loader):
            ...
            if "weak" in ind:
                jem_loss += (joint_entropy_loss(norm(strong_head_1), norm(weak_head_1.detach()))
                             + joint_entropy_loss(norm(strong_head_2), norm(weak_head_2.detach()))) / 2.0
            if "common" in ind:
                jem_loss += (joint_entropy_loss(norm(strong_head_1), norm(weak_head_1.detach()))
                             + joint_entropy_loss(norm(strong_head_2), norm(weak_head_2.detach()))) / 2.0
            if jem_loss == 0:
                raise ValueError(f"Wrong modality for cross-modality indepentisation : {ind}")
...
def parse_args(argv):
    ...
    parser.add_argument(
        '-i', '--independance', nargs="+", type=str, default="weak", choices=["weak", "common"],
         help="Latent space with which the independisation is forced with the strong specific latent space. "
              "Jem loss is computed between strong representations and weak or common (or both) representations."
              "Default is : 'weak'")
    parser.add_argument(
...
def main(argv):
    ...
    # train model
    predictions = train(weak_encoder, strong_encoder, train_loader, test_loader, n_epochs, pond, ind_space)
