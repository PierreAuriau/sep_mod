...
class SepMod(nn.Module):
    ...
    def __init__(self, latent_dim=16, 
        ...
        self.encoders = nn.ModuleDict({
            "skeleton": Encoder(latent_dim=latent_dim//2),
            "image": Encoder(latent_dim=latent_dim//2),
            "shared": Encoder(latent_dim=latent_dim//2)
        })
        self.decoders = nn.ModuleDict({
            "skeleton": Decoder(latent_dim=latent_dim),
            "image": Decoder(latent_dim=latent_dim)
        })        
    ...
    def training_step(self, inputs):
        ...
        if self.tc_loss_between_shared_and_specific or self.tc_loss_between_specifics:
            ...
            # if self.tc_loss_between_specifics:
            #     z_specific = torch.cat((z["image"][:, (self.latent_dim//2):] ,
            #                             z["skeleton"][:, (self.latent_dim//2):]), 
            #                             dim=1)
    ...
    def get_reconstructions(self, loader):
        ...
        for mod in ("image", "skeleton"):
            ...
            outputs = (np.clip(outputs, -1, 1)*0.5 + 0.5) * 255
